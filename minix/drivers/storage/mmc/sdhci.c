...
void sdhci_reset(struct sdhci_host *host, u8 mask)
{
	...
	while (sdhci_readb(host, SDHCI_SOFTWARE_RESET) & mask) {
		if (timeout == 0) {
			log_info(&log, "%s: Reset 0x%x never completed.\n",
			       __func__, (int)mask);
			return;
		}
		timeout--;
...
static int sdhci_transfer_data(struct sdhci_host *host, struct mmc_data *data)
{
	...
	do {
		...
		if (stat & SDHCI_INT_ERROR) {
			log_warn(&log, "%s: Error detected in status(0x%X)!\n",
				 __func__, stat);
			return -EIO;
		}
			      ...
			      struct mmc_data *data)
{
	...
	while (sdhci_readl(host, SDHCI_PRESENT_STATE) & mask) {
		if (time >= cmd_timeout) {
			log_warn(&log, "%s: MMC: busy ", __func__);
			if (2 * cmd_timeout <= SDHCI_CMD_MAX_TIMEOUT) {
				cmd_timeout += cmd_timeout;
				log_warn(&log, "timeout increasing to: %u ms.\n",
				       cmd_timeout);
			} else {
	...
	do {
		...
		if (stat & SDHCI_INT_ERROR)
			break;
		read_frclock(&end);
		if (delta_frclock(start, end) >= SDHCI_READ_STATUS_TIMEOUT * MILLA_TO_MICRO) {
			if (host->quirks & SDHCI_QUIRK_BROKEN_R1B) {
				return 0;
			} else {
				log_warn(&log, "%s: Timeout for status update!\n",
				       __func__);
				return -ETIMEDOUT;
			}
		}
...
int sdhci_init(struct mmc *mmc)
{
	...
	if ((host->quirks & SDHCI_QUIRK_32BIT_DMA_ADDR) && !aligned_buffer) {
		rc = posix_memalign(&aligned_buffer, 8, 512*1024);
		if (rc) {
			log_warn(&log, "%s: Aligned buffer alloc failed!!!\n",
			       __func__);
			return -ENOMEM;
		}
	}

	sdhci_set_power(host, fls32(mmc->cfg->voltages) - 1);
